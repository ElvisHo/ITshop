...
<div class="parent">
  ...
  <div class="container">
    <div class="panel rt">
      <form id="form-register" method="post" action="registshopkeeper.do">
        <div class="txt">
          <p>新店铺注册
            <span>
          ...
          <label for="uname">店铺名：</label>
          <input autocomplete="" required="" minlength="6" maxlength="12" type="text" placeholder="请输入商铺名" 
                      autofocus=""  name="shopname" id="shopname"  />
          <span class="shopmsg-error" id="shopmsg-error">店铺名不能为空且提交不能更改</span>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          ...
          <input required="" type="password" minlength="6" maxlength="12" placeholder="请输入密码"  
                         name="password" autofocus="" id="upwd">
		 ...
		 <div class="form-group">
          ...
          <input required="" type="password" minlength="6" maxlength="12" placeholder="请确认密码" 
                         name="upwdconfirm" autofocus="" id="upwdconfirm">
          <span class="msg-default hidden">密码长度在6到12位之间</span>
          <input type="hidden"  value="s" id="type" name="type"/>
        </div>
...
<script>
 ...
 shopname.onblur = function(){
	    if(this.validity.valueMissing){
	      this.nextElementSibling.innerHTML = '店铺名不能为空';
	      this.nextElementSibling.className = 'shopmsg-error';
	      this.setCustomValidity('用户名不能为空');
	    }else if(this.validity.tooShort){
	      this.nextElementSibling.innerHTML = '店铺名不能少于6位';
	      this.nextElementSibling.className = 'shopmsg-error';
	      this.setCustomValidity('用户名不能少于6位');
	    }else {
	      this.nextElementSibling.innerHTML = '店铺名格式正确';
	      this.nextElementSibling.className = 'msg-success';
	      this.setCustomValidity('');
	      var data ={
	      ...
	      $.post("registshopname_detect.do",data,function(result){
	    	 console.log(result);
	    	 if(result.state==1){
	    	 $("#shopmsg-error").html(result.data);
	    	 }else{
	    		 $("#shopmsg-error").html(result.message);
	    	 }
	      });
	    }
	  }
 ...
 shopname.onfocus = function(){
	    this.nextElementSibling.innerHTML = '店铺名长度在6到9位之间';
	    this.nextElementSibling.className = 'msg-default';
	  }
